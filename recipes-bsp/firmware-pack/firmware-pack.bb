...
do_compile () {
    ...
    if [ "${PMIC_SUPPORT}" = "1" ]; then
		bootparameter ${DEPLOY_DIR}/images/${MACHINE}/bl2-${MACHINE}_pmic.bin bl2_bp_pmic.bin
		cat ${DEPLOY_DIR}/images/${MACHINE}/bl2-${MACHINE}_pmic.bin >> bl2_bp_pmic.bin
		fiptool create --align 16 --soc-fw ${DEPLOY_DIR}/images/${MACHINE}/bl31-${MACHINE}_pmic.bin --nt-fw ${WORKDIR}/recipe-sysroot/boot/u-boot.bin fip_pmic.bin
		objcopy -O srec --adjust-vma=0x00011E00 --srec-forceS3 -I binary bl2_bp_pmic.bin bl2_bp_pmic.srec
		objcopy -I binary -O srec --adjust-vma=0x00000000 --srec-forceS3 fip_pmic.bin fip_pmic.srec
	fi
